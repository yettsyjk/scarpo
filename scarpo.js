...
PlatformManager.prototype.updateWhenLose = function(){
    ...
    this.third.x = (this.second.x + this.second.width)+ random(
        this.maxDistanceBetween - 150, this.maxDistanceBetween
    );
};
...
function Particle(options){
    ...
    this.y = options.y;
    this.size = 10;
    this.velocityX = options.velocityX || random(-(InfiniteRunner.acceleration * 3)
    + -8, -(InfiniteRunner.acceleration * 3));
    this.velocityY = options.velocityY || random(-(
        InfiniteRunner.acceleration * 3
    ));
    this.color = options.color;
}
...
Particle.prototype.draw = function(){
    InfiniteRunner.fillStyle = this.color;
    InfiniteRunner.fillRect(this.x, this.y, this.size, this,this.size);
};

InfiniteRunner.setup = function(){
    this.jumpCount = 0;
    this.acceleration = 0;
    this.accelerationMaker = 0;
    ...
    this.collidePlatform = null;
    this.scoreColor = '#ed8a66';
...
InfiniteRunner.update = function(){
    ...
    switch(this.jumpCount){
        case 10:
            this.accelerationMaker = 1;
            this.platformManager.maxDistanceBetween = 430;
            this.scoreColor = '#a067eb';
            break;
        case 25:
            this.accelerationMaker = 2;
            this.platformManager.maxDistanceBetween = 530;
            this.scoreColor = '#4371a1';
            break;
        case 40:
            this.accelerationMaker = 3;
            this.platformManager.maxDistanceBetween = 580;
            this.scoreColor = '#161618';
            break;
    }

    this.acceleration += (this.accelerationMaker - this.acceleration) * 0.02;
    for (i = 0; i < this.platformManager.platforms.length; i++){
        if (this.player.intersects(this.platformManager.platforms[i] ) ){   
            this.collidePlatform = this.platformManager.platforms[i];
        }
        ...
        color: this.collidePlatform.color
    });
    if (this.player.intersectsLeft(this.platformManager.platforms[i])){
        this.player.x = this.collidePlatform.x -64;
        for (i = 0; i < 10; i++){
            this.particles[(this.particlesIndex++)%this.particlesMax] = new Particle({
                ...
                color: randomChoice([
                    '#f39385', '#d2e2e1', this.collidePlatform.color
                ])
            });
        };
...
InfiniteRunner.draw = function(){
    ...
    this.fillStyle = '#161618';
    this.fillText('RECORD: '+this.jumpCountRecord, this.width - (150 + this.acceleration * 4)),
    this.fillStyle = this.scoreColor;
    this.font = (12 + (this.acceleration * 3)) + 'pt Arial';
    this.fillText('JUMPS: ' +this.jumpCount, this.width - (150 + (this.acceleration * 4) ), 50);
};